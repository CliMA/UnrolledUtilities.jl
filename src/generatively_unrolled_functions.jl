...
# For some reason, passing a StaticOneTo(256) to the methods for
# gen_unrolled_reduce defined above causes compilation of the non-orographic
# gravity wave parametrization test in ClimaAtmos to hang, while passing a
# Val(256) to these methods for val_unrolled_reduce prevents it from hanging.
# However, writing these methods in the same style as gen_unrolled_reduce (using
# Base.Cartesian.@nexprs) still causes compilation to hang. Wrapping the first
# method's result in a block and adding an inline annotation also causes
